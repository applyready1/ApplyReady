...
function parseProjectsSection(lines, resume) {
  ...
  for (const line of lines) {
    ...
    // 1. Inline Project (e.g. "• Project Name: Built a system...")
    if (inlineProject && (!currentProject || currentProject.bullets.length > 0 || hadLeadingBullet)) {
      if (currentProject) projects.push(currentProject);
      currentProject = { name: inlineProject.name, description: '', bullets: [inlineProject.description], link: '' };
      continue;
    }

    // 2. Standalone Project Name
    if (!hadLeadingBullet && isProjectNameLine(cleaned)) {
      if (currentProject && currentProject.bullets.length > 0) {
         projects.push(currentProject);
         currentProject = { name: cleaned, description: '', bullets: [], link: '' };
      } else if (!currentProject) {
         currentProject = { name: cleaned, description: '', bullets: [], link: '' };
      } else {
         // Current project exists but has no bullets yet. Likely a subtitle (tech stack, role, etc.)
         currentProject.description += (currentProject.description ? '\n' : '') + cleaned;
      }
      continue;
    }

    // 3. Content line (description or bullet)
    if (currentProject) {
      currentProject.bullets.push(cleaned);
    } else {
      currentProject = { name: 'Project', description: '', bullets: [cleaned], link: '' };
    }
  }
  ...
  // Transform into final models and verify duplicates
  for (const proj of projects) {
     const finalDesc = [proj.description, ...proj.bullets].filter(Boolean).join('\n');
     
     const isDuplicate = resume.projects.some(existing => 
       (existing.name || '').toLowerCase() === proj.name.toLowerCase() && 
       (existing.description || '') === finalDesc
     );
     
     if (!isDuplicate) {
       resume.projects.push(createProject(proj.name, finalDesc, proj.link));
     }
  }
}
...
function isProjectNameLine(line) {
  ...
  // Reject lines that are obviously sub-headers or tech stacks
  if (/^(technologies|tech stack|tools|skills|environment|github|source code|live demo|link|role|responsibilities|description|features)\b/i.test(cleaned)) {
    return false;
  }

  return (
    /^[A-Za-z0-9]/.test(cleaned) && // Allow starting with number for things like "2D Game Engine"
    cleaned.length > 2 &&
    cleaned.length < 110 &&
    wordCount <= 14 &&
    !/[.!?]$/.test(cleaned) &&
