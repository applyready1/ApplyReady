...
function loadScript(src) {
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      ...
      const fallbackTimer = window.setTimeout(resolve, 800);
      existing.addEventListener('load', resolve, { once: true });
      existing.addEventListener('error', reject, { once: true });
      existing.addEventListener('load', () => window.clearTimeout(fallbackTimer), { once: true });
      existing.addEventListener('error', () => window.clearTimeout(fallbackTimer), { once: true });
      return;
    }
...
async function getJsPdfConstructor() {
  ...
  if (!window.jspdf?.jsPDF) {
    throw new Error('PDF generator is still loading. Try again in a moment.');
  }

  return window.jspdf.jsPDF;
}

function buildResumePDFv2(resumeData, options = {}, jsPDF) {
  const resume = {
    contact: {},
    summary: '',
    skills: [],
...
export async function downloadResumePDFv2(resume, jobTitle = '', company = '') {
  const jsPDF = await getJsPdfConstructor();
  const doc = buildResumePDFv2(resume, {}, jsPDF);

  const name = (resume?.contact?.name || 'Resume').replace(/[^a-zA-Z\s]/g, '').trim().replace(/\s+/g, '_') || 'Resume';
